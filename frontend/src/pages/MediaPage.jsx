...
export default function MediaPage() {
  ...
  const coverUrl = media.coverImage 
    ? `${import.meta.env.VITE_API_URL || ''}/${media.coverImage}` 
    : null;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-8">
        {coverUrl ? (
          <img src={coverUrl} alt={media.title} className="w-64 h-96 object-cover rounded-lg shadow-lg" />
        ) : (
        ...
        <div>
          ...
          <p className="text-gray-500 mt-4">{media.items.length} episodes</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {media.items.map((item, idx) => (
          <Link to={`/play/${item.id}?mediaId=${media.id}`} key={item.id} className="bg-gray-800 p-4 rounded hover:bg-gray-700 transition flex justify-between items-center">
            <div>
